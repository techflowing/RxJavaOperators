...
//        refCount();
    ...
    private static void refCount() {
        ...
        Subscription subscription = connectableObservable.refCount().subscribe(new Action1<Long>() {
            @Override
    ...
    private static void replay() {
        final ConnectableObservable<Long> countLimitObservable = Observable.interval(1, TimeUnit.SECONDS)
                .observeOn(Schedulers.newThread())
                .replay(2);

        ConnectableObservable<Long> timeLimitObservable = Observable.interval(1, TimeUnit.SECONDS)
                .observeOn(Schedulers.newThread())
