...
import win.techflowing.rxjava.SubscriberFactory;
...
//        groupJoin();
    ...
    private static void startWith() {
        ...
        Observable.just(4, 5, 6)
                ...
                .startWith(ob)
                .subscribe(SubscriberFactory.getIntegerSubscriber("startWith()"));
    }


    /**
     * Merge操作符将多个Observable发射的数据整合起来发射，但是其发射的数据有可能是交错的，
     * 如果想要没有交错，可以使用concat操作符。
    ...
    private static void mergeDelayError() {
        ...
        })).subscribe(SubscriberFactory.getIntegerSubscriber("mergeDelayError()"));
    }
    ...
    private static void combineLatest() {
        Observable<Long> observable1 = Observable.timer(0, 1000, TimeUnit.MILLISECONDS).take(5);
        Observable<Long> observable2 = Observable.timer(0, 2100, TimeUnit.MILLISECONDS).take(5);
    ...
    private static void join() {
        Observable<Long> observable1 = Observable.timer(0, 500, TimeUnit.MILLISECONDS).take(5);
        Observable<Long> observable2 = Observable.timer(0, 1000, TimeUnit.MILLISECONDS).take(5);
    ...
    private static void groupJoin() {
        ...
        observable1.groupJoin(observable2, new Func1<Long, Observable<Long>>() {
            ...
            @Override
            public Observable<Long> call(final Long aLong1, Observable<Long> longObservable) {
            ...
            public void onNext(Observable<Long> longObservable) {
                longObservable.subscribe(new Subscriber<Long>() {
                    ...
                    @Override
                    public void onError(Throwable e) {
                        System.out.println("groupJoin()-onNext() error:" + e.getMessage());
                    }

                    @Override
                    public void onNext(Long aLong) {
                        System.out.println("groupJoin()-onNext() onNext:" + aLong);
                    }
                });
            }
        });
     ...
     * 转换规则为：对于这组Observable中的每一个Observable所产生的结果，如果在同一个时间内存在两个或多个Observable提交的结果，
    ...
    private static void switchOnNext() {
        Observable<Observable<Long>> observable = Observable.timer(0, 500, TimeUnit.MILLISECONDS)
                .map(new Func1<Long, Observable<Long>>() {
                    @Override
                    public Observable<Long> call(Long aLong) {
                        return Observable.timer(0, 200, TimeUnit.MILLISECONDS).take(5);
                    }
                }).take(2);
