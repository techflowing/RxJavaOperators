...
public class Transforming {
    ...
    private static void flatMapIterable() {
        Observable.just(2, 3, 4, 5)
                ...
                .subscribe(new Subscriber<Object>() {
                    ...
                    @Override
                    public void onError(Throwable e) {
                        System.out.println("flatMapIterable() error:" + e.getMessage());
                    }

                    @Override
                    public void onNext(Object o) {
                        System.out.println("flatMapIterable() onNext:" + o.toString());
                    }
                });
    }
    ...
    private static void concatMap() {
        Observable.just(1, 2, 3, 4, 5)
                ...
                .subscribe(new Subscriber<Object>() {
                    ...
                    @Override
                    public void onError(Throwable e) {
                        System.out.println("concatMap() error:" + e.getMessage());
                    }
    ...
    private static void switchMap() {
        Observable.just(1, 2, 3, 4, 5, 6)
                .switchMap(new Func1<Integer, Observable<?>>() {
                    @Override
                    public Observable<?> call(Integer integer) {
                        return Observable.just("switchMap后：" + integer).delay(100, TimeUnit.MILLISECONDS);
                    }
                })
                .subscribe(new Subscriber<Object>() {
                    @Override
                    ...
                    @Override
                    public void onError(Throwable e) {
                        System.out.println("switchMap() error:" + e.getMessage());
                    }
        ...
        Observable.just(1, 2, 3, 4, 5, 6)
                .scan(new Func2<Integer, Integer, Integer>() {
                    ...
                    public Integer call(Integer sum, Integer item) {
                        System.out.println("上次计算的结果：" + sum + " ，源Observable发出的数据：" + item);
                        return sum + item;
                    }
                })
                .subscribe(new Subscriber<Integer>() {
    ...
    private static void groupBy() {
        Observable.just(1, 2, 3, 4, 5, 6)
                ...
                .subscribe(new Subscriber<GroupedObservable<Integer, Integer>>() {
                    ...
                    @Override
                    public void onError(Throwable e) {
                        System.out.println("groupBy() error:" + e.getMessage());
                    }
                    ...
                    public void onNext(final GroupedObservable<Integer, Integer> integerIntegerGroupedObservable) {
                        integerIntegerGroupedObservable.subscribe(new Action1<Integer>() {
                            ...
                            public void call(Integer integer) {
                                System.out.println("groupBy() key:" + integerIntegerGroupedObservable.getKey() + " " +
                                        "value:" + integer);
                            }
                        });
                    }
    ...
    /**
     * Buffer操作符,要做的事情就是将数据安装规定的大小做一下缓存，然后将缓存的数据作为一个List集合发射出去
     * skip 参数可选，指定每次发射一个集合需要跳过几个数据
     * 可以根据时间或者数目分组
     */
        ...
        Observable.just(1, 2, 3, 4, 5, 6, 7, 8, 9)
                ...
                .subscribe(new Subscriber<List<Integer>>() {
                    ...
                    public void onNext(List<Integer> integers) {
                        System.out.println("buffer() onNext:" + integers.toString());
                    }
                });
    }
